...
// input bool  multi_op = false;                                              //Multi Operacao (define o m�todo de operacao)
input ENUM_TIMEFRAMES TimeFrame = PERIOD_M10;                              //TimeFrame base

input int HoraDeInicio = 9;                                                //Hora de In�cio
input int MinutoDeInicio = 20;                                             //Minuto de Inicio
input int HoraDeFim = 17;                                                  //Hora de Fim
input int MinutoDeFim = 27;                                                //Minuto de Fim
input bool   ZerarFinalDoDia = true;                                       //Encerra opera��es no final do dia (execu��o extendida)
input string Descricao_Robo_Alpha = "";                                    //Descri��o para logs e mensagens
...
input int Limite_Operacoes = 9999;                                         //Limite de opera��es (entrada e Saida)
...
input bool Otimizacao = false;                                          //Parametro para otimiza��o
...
input double RAW_MoverSL = 0;                                              //Mover o StopLoss DELTA (dist�ncia da entrada, 0 desliga)
input double PontoDeMudancaSL = 0;                                         //Distancia da entrada DELTA (Dire��o do Lucro, 0 = Preco da Opera��o)
...
input Met_Prop Prop_Metodo = 534;                                         //M�todo utilizado para o DELTA
input int      Prop_Periodos = 3;                                         //Per�odos do prop SMA(3) BB(20)
input double   Prop_StopLoss = 0.7;                                       //StopLoss: Multiplicador do Delta (0 desliga)
input double   Prop_MoverSL = 0;                                          //Mover o StopLoss DELTA (dist�ncia da entrada, 0 desliga)
input double   Prop_PontoDeMudancaSL = 0;                                 //Distancia da entrada DELTA (Dire��o do Lucro, 0 = Preco da Opera��o)
...
input double   Prop_Limite_Minimo_Tick_Size = 0;                          //Limite M�nimo para operar (*Tick Size)
