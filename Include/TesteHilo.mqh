...
//--- get handles
   ma_high_handle=iMA(NULL,0,period2,0,InpMethod,PRICE_HIGH);
   ma_low_handle =iMA(NULL,0,period2,0,InpMethod,PRICE_LOW);
...
//--- set indicator properties
   ...
   string short_name=StringFormat("Gann High-Low Activator SSL (%u, %s)",period2,StringSubstr(EnumToString(InpMethod),5));
   IndicatorSetString(INDICATOR_SHORTNAME,short_name);
   IndicatorSetInteger(INDICATOR_DIGITS,_Digits);
   
   
//--- set label
   short_name=StringFormat("GHL (%u, %s)",period2,StringSubstr(EnumToString(InpMethod),5));
   PlotIndexSetString(0,PLOT_LABEL,short_name);
//--- done
 //  Tendencia = TrendBuffer;
   //Tendencia2 = PLOT_LABEL;
...
//+------------------------------------------------------------------+
/*
int Calcula(const int rates_total,
                ...
                const int &spread[])
  {
   if(rates_total<period2+1)return(0);
