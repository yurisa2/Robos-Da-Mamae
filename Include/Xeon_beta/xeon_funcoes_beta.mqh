...
void Xeon_beta::Get_Data(int barra, int& Array_Size, double& Valores[])
{
  ...
  if(delta_atual > 0  && delta_atual > (xeon_min_diff * Tick_Size)) Valores[n++] = 1;
  else Valores[n++] = 0;
...
void Xeon_beta::Avalia() {
  ...
  bool horario_perm = Condicoes.Horario();
  ...
  if(horario_perm) {
    ...
    if(processado[1] > xeon_thresh && processado[0] > 16 &&
       (processado[0] + xeon_delta) < processado[1])  {
        if(!Otimizacao)  Print("direcao_now: " + IntegerToString(direcao_now) +
                                " | Vai: 1 - v0: " + DoubleToString(processado[0],2) +
                                " | v1: " + DoubleToString(processado[1],2));
        status_exchange = 1;
      }
      ...
      if(!Otimizacao) Print("Fora de Horario");
    }
